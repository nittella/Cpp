...
InternalAtomsCoordinates ConvertToInternal(
    ...
    while (!stack.empty()){
        ...
        for(int i = 0; i < dm1.Size() - 1; i++){
            ...
            if(dm1.GetDistance(atom, atom_i) < MaxBoundLength()){
    ...
    for(auto [bond, v] : bond_to_distance){
        ...
        for (int i = 0; i < v.size(); i++){
            ...
            if(set<Atom>{atom1, atom2, atom3, atom4}.size() == 4){
                    if(CalculateDistance(c_cords.GetCoordinate(atom2.number),
                                         c_cords.GetCoordinate(atom3.number))
                                         > MaxBoundLength()){
                    Atom tmp = atom1;
                    atom1 = atom2;
                    atom2 = tmp;
