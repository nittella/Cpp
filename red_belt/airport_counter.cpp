...
public:
  ...
  static const uint32_t AIRPORTS_COUNT = static_cast<uint32_t>(TAirport::Last_);
  // конструктор по умолчанию: список элементов пока пуст
  AirportCounter(){
      for (uint32_t i = 0; i < AIRPORTS_COUNT; i++ ){
          data_[i] = 0;
      }
  };
  ...
  using Items = array<Item, (AIRPORTS_COUNT)>;
  ...
  Items GetItems() const{
      Items result;
      for (uint32_t i = 0; i < AIRPORTS_COUNT; i++ ){
          result[i] = make_pair(TAirport(i), data_.at(i));
      }
      return result;
  };

private:
  array<uint32_t, (AIRPORTS_COUNT)> data_;
};
